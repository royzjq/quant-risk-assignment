...
def validate_exec_csv(file_path="exec.csv"):
    ...
    for col in df.columns:
        ...
        if len(missing_indices) > 0:
            ...
            if 'Unnamed: 0' in df.columns:
                ...
                results["missing_values"][col]["csv_row_numbers"] = [int(i) for i in missing_indices] # Use pandas index if no 'Unnamed: 0'
    ...
    categorical_cols = ['EventType', 'ech', 'sym', 'sde'] # Not directly used for conversion, but for context

    for col in numerical_cols:
        initial_missing_count = df[col].isnull().sum()
        df[col] = pd.to_numeric(df[col], errors='coerce')
        non_numeric_after_coerce_indices = df[df[col].isnull() & (df[col].index.isin(df.index.difference(df.loc[df[col].isnull()].index)))].index.tolist() # Indices that became NaN due to coercion, not initially NaN
    ...
    if 'ech' in df.columns:
        expected_ech_types = ['Binance'] # Based on initial inspection
    ...
    # sde check (should be 1 or 2, 0 is unexpected but reported)
    if 'sde' in df.columns:
        df['sde'] = pd.to_numeric(df['sde'], errors='coerce')
        # Values that are not 1, 2, or NaN (due to coercion) are invalid
    ...
    # Price and Quantity related checks (px, qty, epx, eqty, vwap, rqty, fee)
    ...
    # Fee check: should be strictly negative
    ...
    # Consistency check: qty vs eqty vs ceqty
    if all(col in df.columns for col in ['qty', 'eqty', 'ceqty', 'rqty']) and \
       all(pd.api.types.is_numeric_dtype(df[col]) for col in ['qty', 'eqty', 'ceqty', 'rqty']):
        ...
        inconsistent_rows = []
        inconsistency_reasons = {}  # To store the reason for each inconsistent row
        ...
        # Second check: eqty + rqty != qty, with additional logic
        # Use tolerance for floating point comparison here as well
        ...
        if not eqty_plus_rqty_ne_qty.empty:
            for idx in eqty_plus_rqty_ne_qty.index:
                # Skip if already marked as inconsistent
                ...
                # Find the previous row with the same sym
                ...
                if not same_sym_before_current.empty:
                    # Get the most recent previous row with same sym
                    prev_row = same_sym_before_current.iloc[-1]
                    prev_rqty = prev_row['rqty']
                    
                    # If current (eqty + rqty) != previous rqty, mark as inconsistent
                    ...
                    # No previous row with same sym found, mark as inconsistent
                    inconsistent_rows.append(idx)
                    inconsistency_reasons[idx] = "No previous row with same sym found"
        
        # Remove duplicates and create final inconsistent DataFrame
        ...
        if not inconsistent_qty_df.empty:
            results["anomalies"]["qty_consistency"] = {
                ...
                "all_indices": [int(i) for i in inconsistent_indices], # Ensure int conversion here
                "all_data": inconsistent_qty_df.to_dict(orient='records'),
                "all_reasons": [inconsistency_reasons[idx] for idx in inconsistent_indices],
                "all_csv_row_numbers": [int(df.loc[idx, 'Unnamed: 0']) if 'Unnamed: 0' in df.columns else int(idx) for idx in inconsistent_indices] # Ensure int conversion here
            }
            print(f"  - Quantity Consistency: {len(inconsistent_qty_df)} inconsistent rows found (eqty > qty or rqty inconsistency with previous row of same sym).")
        else:
            print("  - Quantity Consistency: qty, eqty, rqty values are consistent.")

    print("\nValidation complete. Summary:")
    # The printing part can be simplified since we are saving to JSON
    print("Generating validation_report.json...")

    # Save results to a JSON file
    output_json_file = "exec_validation.json"
...
if __name__ == "__main__":
    validation_results = validate_exec_csv()
    # The printing of results here is now redundant as it's done within the function and saved to JSON. 